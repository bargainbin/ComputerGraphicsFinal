...
public class MVRWandRaycaster : BaseRaycaster
{
    private RectTransform _rectTransform;
    private Camera _wandCamera;
    ...
    public override Camera eventCamera
    {
        get
        {
            if (_wandCamera == null)
            {
                var wand = FindObjectOfType<MVRWand>();
                _wandCamera = wand.GetComponent<Camera>();
                if (_wandCamera == null)
                {
                    _wandCamera = wand.gameObject.AddComponent<Camera>();
                    _wandCamera.enabled = false;
                }
            }
    ...
    public override void Raycast(PointerEventData eventData, List<RaycastResult> resultAppendList)
    {
        ...
        {
            var canvasHitPoint = _rectTransform.InverseTransformPoint(hitPosition);
    ...
    {
        Vector3 vertex0 = new Vector3(_rectTransform.rect.xMin, _rectTransform.rect.yMin);
        Vector3 vertex1 = new Vector3(_rectTransform.rect.xMax, _rectTransform.rect.yMin);
        Vector3 vertex2 = new Vector3(_rectTransform.rect.xMin, _rectTransform.rect.yMax);
        Vector3 vertex3 = new Vector3(_rectTransform.rect.xMax, _rectTransform.rect.yMax);

        if (RaycastTriangle(_rectTransform.transform.TransformPoint(vertex0),
                            _rectTransform.transform.TransformPoint(vertex1),
                            _rectTransform.transform.TransformPoint(vertex2),
                            iRay, iMaxDist, out oHitResult, out oHitPosition))
        {
            return true;
        }

        return RaycastTriangle(_rectTransform.transform.TransformPoint(vertex1),
                               _rectTransform.transform.TransformPoint(vertex2),
                               _rectTransform.transform.TransformPoint(vertex3),
                               iRay, iMaxDist, out oHitResult, out oHitPosition);
    }
